...
class MarioAgent:
    ...
    def build_model(self):
        ...
        # model.add(Conv2D(512, 16, kernel_initializer=self.initializer, strides=4, input_shape=self.state_space))
        model.add(Activation("selu"))
        model.add(Conv2D(64, 4, kernel_initializer=self.initializer, strides=2))
        # model.add(Conv2D(1024, 8, kernel_initializer=self.initializer, strides=2))
        model.add(Activation("selu"))
        model.add(Conv2D(64, 3, kernel_initializer=self.initializer, strides=1))
        # model.add(Conv2D(2048, 6, kernel_initializer=self.initializer, strides=1))
        model.add(Activation("selu"))
        model.add(Flatten())
        model.add(Dense(256, kernel_initializer=self.initializer, activation="selu"))
        model.add(Dense(128, kernel_initializer=self.initializer, activation="selu"))
        # model.add(Dense(4196, kernel_initializer=self.initializer, activation="selu"))
        # model.add(Dense(2048, kernel_initializer=self.initializer, activation="selu"))
